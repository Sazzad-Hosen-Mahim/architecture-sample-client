...
export default function PhaseMeetingCards({ project }: PhaseMeetingCardsProps) {
    const { data: proposalsData } = useGetProposalsByProjectRequestQuery(project.id);
    const { data: paymentsData } = useGetProjectPaymentsQuery(project.id);

    const acceptedProposals = (proposalsData?.data || []).filter((p) => p.status === "ACCEPTED");
    ...
    return (
        <div className="space-y-4 pt-4 border-t border-gray-100">
            <h3 className="text-sm font-bold text-gray-700 uppercase tracking-widest">Phase Meetings</h3>
            {acceptedProposals.map((proposal) => (
                <ProposalPhaseMeetings key={proposal.id} project={project} proposal={proposal} payments={payments} />
            ))}
        </div>
    );
}
...
function ProposalPhaseMeetings({
    ...
    return (
        <div className="space-y-2">
            <p className="text-xs font-semibold text-gray-500">{proposal.title || proposal.projectName}</p>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {stages.map((stage: any) => {
                    const stageMeeting = project.meetingLinks?.find((m: any) => m.stageId === stage.id);
                    const paid = payments.some((p) => p.stageId === stage.id && p.paymentStatus === "COMPLETED");

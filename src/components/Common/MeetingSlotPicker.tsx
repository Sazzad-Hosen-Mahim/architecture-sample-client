...
export default function MeetingSlotPicker({
  ...
  const slots: DaySlot[] = useMemo(
    () =>
      selectedDay ? buildDaySlots(selectedDay, busy, { officeHours }) : [],
  ...
  const endOptions = useMemo(() => {
    ...
    for (let m = value.startMinutes; m < 24 * 60; m += SLOT_MINUTES) {
    ...
    <div className="space-y-3">
      ...
      {!value.date ? (
        ...
        <div className="flex items-start gap-2 text-xs text-amber-800 bg-amber-50 border border-amber-200 rounded-lg px-3 py-4">
          ...
          <span>The office is closed that day. Please pick another day.</span>
        </div>
      ) : (
        <div>
          ...
          <p className="text-[11px] text-gray-500 mb-2">
            Or click a start time below, then an end time. Greyed-out times are
            already booked or blocked off.
          </p>
            ...
            {slots.map((slot) => {
              ...
              return (
                <button
                  ...
                  className={`px-1.5 py-1.5 rounded-md border text-[11px] font-medium transition-colors ${
                    unavailable
          ...
          {slots.some((s) => s.tentative) && (
            <p className="text-[11px] text-amber-700 mt-2 flex items-center gap-1">
              ...
              Amber times have a request awaiting confirmation — still bookable.
            </p>
          )}

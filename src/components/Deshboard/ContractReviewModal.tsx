...
/**
 * The architect's signature image for a contract.
 *
 * There are two places it can live: `architectContractSignature`, written when
 * the PM signs and sends the proposal from the wizard, and `architectSignature`,
 * written by the separate `/proposals/:id/sign` route. A contract signed
 * through the second path — or before the contract field existed — would
 * otherwise render "Pending signature" to the client despite being signed.
 */
const architectSignatureOf = (contract: any): string | null =>
  contract?.architectContractSignature || contract?.architectSignature || null;
    ...
    <Page size="A4" style={pdfStyles.page}>
      ...
      <View style={pdfStyles.greeting}>
        ...
        <Text style={{ marginTop: 10 }}>
          ...
          {contract?.serviceType || "New Construction"} at{" "}
          {contract?.projectLocation || "the project site"}.
        </Text>
      </View>
      ...
      {sections.map((section) => {
        ...
        return (
          <View key={section.articleKey} style={{ marginBottom: 10 }}>
            <Text style={pdfStyles.articleTitle} wrap={false}>
              {section.title}
            </Text>
            ...
            {isPayment && services.length > 0 && (
              <View style={{ marginBottom: 12 }}>
                <Text style={[pdfStyles.bold, { marginBottom: 6 }]}>
                  3.01 Payment Structure
                </Text>
                ...
                <View style={pdfStyles.table} wrap={false}>
                  <View style={pdfStyles.tableHeader}>
                    <View style={{ width: 50 }}>
                      <Text style={pdfStyles.bold}>ORDER #</Text>
                    </View>
                    <View style={pdfStyles.tableCellLeft}>
                      <Text style={pdfStyles.bold}>
                  ...
                  {services.map((s: any, idx: number) => (
                    <View key={`structure-${s.id}`} style={pdfStyles.tableRow}>
                      <View style={{ width: 50 }}>
                        <Text>{s.order ?? idx + 1}</Text>
                      </View>
                      <View style={pdfStyles.tableCellLeft}>
                        <Text>{s.name}</Text>
                      </View>
                      <View style={pdfStyles.tableCellRight}>
                        <Text>${Number(s.amount || 0).toLocaleString()}</Text>
                      </View>
                    </View>
                  ))}
                  <View style={pdfStyles.tableFooter}>
                    <View style={pdfStyles.tableCellLeft}>
                      <Text>TOTAL PROFESSIONAL ARCHITECTURAL FEE</Text>
                    </View>
                    <View style={pdfStyles.tableCellRight}>
                      <Text>
                        $
                      ...
                      if (contract.notes.trim().startsWith("{")) {
                        ...
                        const matchingKey = Object.keys(allNotes).find((k) => {
                          ...
                          return (
                            ...
                            key.includes(sName) ||
                            (sTitle &&
                              (sTitle.includes(key) || key.includes(sTitle)))
                          );
                        });
                  ...
                  return (
                    <View key={s.id} wrap={false} style={{ marginBottom: 12 }}>
                      ...
                      >
                        ...
                        >
                          {getServiceSectionHeading(index, s.name)}
                        </Text>
                      </View>
                      {scopeDesc?.intro && (
                      ...
                      {scopeDesc &&
                        scopeDesc.bullets.map((bullet, idx: number) => {
                          const { text, subBullets } = normalizeBullet(bullet);
                          return (
                            <View key={idx} wrap={false}>
                              ...
                              >
                                <Text style={{ width: 12, fontSize: 9 }}>
                                  –
                                </Text>
                                <Text
                                  style={{
      ...
      <View style={pdfStyles.signatureSection} wrap={false}>
        <View style={pdfStyles.signatureBox}>
          ...
          <Text style={pdfStyles.sigName}>
            {contract?.clientName || "Client Signature"}
          </Text>
          <Text style={{ fontSize: 8, color: "#999" }}>
            Date:{" "}
            {contract?.clientContractSignedAt
              ? new Date(contract.clientContractSignedAt).toLocaleDateString()
              : "N/A"}
          </Text>
        </View>
        ...
        <View style={pdfStyles.signatureBox}>
          ...
          {architectSignatureOf(contract) ? (
            <Image
              src={architectSignatureOf(contract)!}
              style={pdfStyles.sigImage}
            />
          ) : (
            <View style={{ height: 50, justifyContent: "center" }}>
...
export default function ContractReviewModal({
  ...
  const {
    ...
    isLoading,
    refetch,
  } = useGetContractForProposalQuery(proposalId, {
    skip: !isOpen || !proposalId,
  });
  const [signContract, { isLoading: isSigning }] =
    useClientSignContractMutation();
  ...
  const originalSections: ContractSection[] = (
    Array.isArray(rawSections)
      ? rawSections
      : typeof rawSections === "string" &&
          (rawSections as string).trim().startsWith("[")
  ...
  if (!hasScope) {
    sections.push({
      ...
      order:
        sections.length > 0
          ? Math.min(...sections.map((s) => s.order)) - 0.5
          : 2,
    });
  }
  if (!hasPayment) {
    sections.push({
      ...
      content:
        "The following terms outline the payment structure for the professional services provided.",
      order:
        sections.length > 0 ? Math.max(...sections.map((s) => s.order)) + 1 : 3,
    });
  }

  // Ensure Project Understanding is present
  const hasProjectUnderstanding = sections.some(
    (s) =>
  ...
  if (!hasProjectUnderstanding) {
    const serviceTypeDisplay =
      contract?.serviceType?.replace(/_/g, " ").toLowerCase() ||
      "new construction";
    ...
    const selectedServiceNames = (contract?.services || []).map(
      (s: any) => s.name?.toLowerCase() || "",
    );
    const servicePhases: string[] = [];
    if (selectedServiceNames.some((n: string) => n.includes("schematic")))
      servicePhases.push("schematic");
    if (selectedServiceNames.some((n: string) => n.includes("development")))
      servicePhases.push("design development");
    if (
      selectedServiceNames.some((n: string) => n.includes("construction doc"))
    )
      servicePhases.push("construction drawings");
    const phasesText =
      servicePhases.length > 0
        ? servicePhases.map((p) => `[${p}]`).join(", ")
        : "[schematic], [design development], [construction drawings]";
...
• "Completion" refers to the final completion of all construction work, including punch list items and final inspections, after which the Project is fully delivered to the Owner.`;
  ...
  const allSectionsRead =
    sections.length > 0 && readSections.size === sections.length;

  const handleSign = async () => {
    if (!allSectionsRead && !isAlreadySigned) {
      toast.error(
        "Please read and mark all sections as complete before signing.",
      );
      return;
    }
          ...
          {isLoading ? (
            ...
            <div className="text-center py-16 space-y-3">
              ...
              <p className="text-gray-500">
                No contract sections attached to this proposal.
              </p>
            </div>
          ) : (
                  ...
                  <div className="flex items-center gap-2">
                    <div className="w-32 h-2 bg-blue-200 rounded-full overflow-hidden">
                      <div
                        ...
                        style={{
                          width: `${(readSections.size / (sections.length || 1)) * 100}%`,
                        }}
                      />
                    </div>
        ...
        <div className="px-6 py-4 border-t border-gray-100 bg-gray-50 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            {isAlreadySigned && (
              <PDFDownloadLink
                document={
                  <ContractPDF contract={contract} sections={sections} />
                }
  ...
  return (
    <div className="space-y-6">
      ...
      {isPayment && services.length > 0 && (
        <div className="space-y-3">
          <h4
            className={`font-semibold text-gray-900 ${isForPdf ? "text-[11pt]" : "text-sm"}`}
          >
            3.01 Payment Structure
          </h4>
          <p
            className={`text-gray-700 leading-relaxed ${isForPdf ? "text-[11pt]" : "text-sm"}`}
          >
            {paymentStructureIntro(contract)}
          </p>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-200">
                  <th
                    className={`px-4 py-3 w-20 font-bold text-gray-600 uppercase tracking-widest ${isForPdf ? "text-[9pt]" : "text-[10px]"}`}
                  >
                    Order #
                  </th>
                  <th
                    className={`px-4 py-3 font-bold text-gray-600 uppercase tracking-widest ${isForPdf ? "text-[9pt]" : "text-[10px]"}`}
                  >
                    Professional Services Fee
                  </th>
                  <th
                    className={`px-4 py-3 font-bold text-gray-600 uppercase tracking-widest text-right ${isForPdf ? "text-[9pt]" : "text-[10px]"}`}
                  >
                    Amount
                  </th>
                </tr>
              ...
              <tbody className="divide-y divide-gray-100 bg-white">
                {services.map((service: any, idx: number) => (
                  <tr key={`structure-${service.id}`}>
                    <td
                      className={`px-4 py-3 text-gray-500 ${isForPdf ? "text-[11pt]" : "text-sm"}`}
                    >
                      {service.order ?? idx + 1}
                    </td>
                    <td
                      className={`px-4 py-3 text-gray-800 ${isForPdf ? "text-[11pt]" : "text-sm"}`}
                    >
                      {service.name}
                    </td>
                    <td
                      className={`px-4 py-3 font-bold text-right text-gray-900 ${isForPdf ? "text-[11pt]" : "text-sm"}`}
                    >
              ...
              <tfoot className="bg-gray-900 text-white">
                <tr>
                  ...
                  <td
                    className={`px-4 py-4 font-bold text-right ${isForPdf ? "text-[12pt]" : "text-base"}`}
                  >
                    $
                    {services
                      .reduce(
      ...
      <p
        className={`text-gray-700 leading-relaxed whitespace-pre-wrap ${isForPdf ? "text-[11pt]" : "text-sm"}`}
      >
        {section.content}
      </p>
      ...
      {isScope && services.length > 0 && (
        <div
          className={`${isForPdf ? "mt-6 py-4 px-6 bg-gray-50 rounded" : "bg-white p-5 rounded-xl border border-gray-100 shadow-sm"}`}
        >
          <h5
            className={`${isForPdf ? "text-sm" : "text-xs"} font-bold text-gray-500 uppercase tracking-widest mb-4 border-b pb-2`}
          >
            Scope of Professional Services
          </h5>
          <div className="space-y-6">
              ...
              return (
                <div key={service.id} className="space-y-2">
                  <div className="flex items-start gap-3">
                    ...
                    />
                    <p
                      className={`${isForPdf ? "text-[11pt]" : "text-sm"} font-bold text-gray-900`}
                    >
                      {getServiceSectionHeading(index, service.name)}
                    </p>
                  </div>
                  {scopeDesc?.intro && (
                    <p
                      className={`ml-7 ${isForPdf ? "text-[10pt]" : "text-xs"} text-gray-700 leading-relaxed`}
                    >
                      {scopeDesc.intro}
                    </p>
                  )}
                  {scopeDesc && (
                    <ul
                      className={`ml-7 space-y-1 ${isForPdf ? "text-[10pt]" : "text-xs"} text-gray-700 list-disc pl-4`}
                    >
                    ...
                    <div className="ml-11 mt-2 space-y-1">
                      ...
                      <ul className="space-y-1">
                        {perServiceNotes.map((note, idx) => (
                          <li
                            key={idx}
                            className="text-xs text-gray-600 bg-amber-50/50 border-l-2 border-amber-200 pl-3 py-1 flex items-start gap-2"
                          >

...
export default function ClientProjectInfoTab({
  ...
  const contractGroups = useMemo(() => {
    ...
    stages.forEach((stage: any) => {
      const key =
        stage.proposalId && byId.has(stage.proposalId)
          ? stage.proposalId
          : ORIGINAL_KEY;
      if (!groups.has(key)) {
      ...
      {stages.length === 0 ? (
        <div className="text-center py-16 bg-gray-50/50 rounded-2xl border border-dashed border-gray-200 mb-8">
          ...
          <p className="text-xs text-gray-400 max-w-[200px] mx-auto italic">
            Individual project phases will appear here once the contract
            ...
            <div key={group.key} className="space-y-5">
              ...
              {group.stages.map((stage: any, idx: number) => {
                ...
                return (
                  <div
                    key={stage.id}
                    className={`group relative border rounded-2xl p-5 transition-all duration-300 hover:shadow-xl hover:-translate-y-0.5 ${
                      isCompleted
                  ...
                  >
                    <div className="flex items-start justify-between gap-4 mb-4">
                      <div className="flex items-start gap-4">
                        <div
                          className={`mt-0.5 w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 border transition-colors ${
                            isCompleted
                        ...
                        <div className="min-w-0">
                          <div className="flex flex-wrap items-center gap-2 mb-1">
                            ...
                            <span
                              className={`text-[8px] font-bold px-1.5 py-0.5 rounded border uppercase tracking-widest ${
                                isCompleted

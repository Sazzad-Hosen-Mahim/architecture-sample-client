import { X, Loader2, Info, FileText, CalendarClock, Paperclip } from "lucide-react";
...
import RefundPhaseSelectModal, { RefundablePhase } from "./RefundPhaseSelectModal";
...
export default function ClientProjectDetailsModal({
    ...
    const { data: latestProject, isLoading, refetch: refetchProject } = useGetProjectRequestByIdQuery(initialProject?.id, {
    ...
    const { data: bankData } = useGetUserBankDetailsQuery(undefined, { skip: !isOpen });
    ...
    const { data: myRefunds } = useGetMyRefundRequestsQuery(undefined, { skip: !isOpen });
    ...
    const [selectedPhasesForRefund, setSelectedPhasesForRefund] = useState<RefundablePhase[]>([]);
    ...
    useEffect(() => {
        if (initialProject?.id) setActiveTab((initialTab as ClientTab) || "details");
        ...
        try {
            ...
            for (const [index, phase] of selectedPhasesForRefund.entries()) {
                await createRefund({
                    ...
                    amount: phase.amount,
                    bankDetails: index === 0 ? pendingBankDetails || undefined : undefined,
                }).unwrap();
            }
    ...
    return (
        <>
                            ...
                            {TABS.map((tab) => {
                                ...
                                return (
                                    <button
                                        ...
                                        className={`inline-flex items-center gap-2 px-3 sm:px-4 py-2.5 text-xs sm:text-sm font-medium border-b-2 transition-all whitespace-nowrap ${isActive
            ...
            <RefundPhaseSelectModal
                ...
                contractTitle={refundContract?.title || project?.projectName || "Contract"}

...
export default function MeetingRequestTab({ project }: MeetingRequestTabProps) {
    ...
    const [attachMeetingLink, { isLoading: isAttaching }] = useAttachMeetingLinkMutation();
    ...
    const handleSendMeetingLink = async (e: React.FormEvent) => {
        ...
        try {
            if (linkingMeeting) {
                ...
                const retimed =
                    window &&
                    window.scheduledAt !== new Date(linkingMeeting.scheduledAt).toISOString();
            ...
            toast.error(
                error?.data?.message || "Failed to send meeting link. Please try again."
            );
        }
    };
    ...
    const startLinking = (meeting: any) => {
        ...
        setMeetingForm({
            ...
            title:
                meeting.title?.includes("Requested by Client")
    ...
    const meetingsFor = (type: MeetingType) =>
        allMeetings.filter((m) => {
            ...
            const isPhase =
                Boolean(m.stageId) || m.meetingType === "PHASE_PROGRESS";
    ...
    const renderMeetingCard = (meeting: any) => {
        ...
        return (
            <div
                key={meeting.id}
                className={`p-4 rounded-xl border transition-all ${isClientRequest
                ...
                <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex items-start gap-4 animate-in fade-in slide-in-from-top-2 duration-500">
                    ...
                    <div className="flex-1">
                        ...
                        <p className="text-xs text-amber-700 mt-1">
                            The client has requested a meeting. Accept or decline the requested
                            date below.
                        </p>
                    </div>
                </div>
                    ...
                    >
                        {linkingMeeting ? (
                            <div className="mb-5 p-3 rounded-lg bg-white border border-blue-200">
                                ...
                                <p className="text-[11px] text-gray-500 mt-1">
                                    The time is already confirmed with the client — just add
                                    the joining link. Changing the time below will ask them to
                                    confirm again.
                                </p>
                                <button
                                    type="button"
